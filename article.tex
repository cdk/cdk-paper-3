...
drug discovery, and the life sciences. It provides various models to represent
chemical structures, of which the chemical graph is essential, and as such
complements quantum chemical approaches. Large scale calculations relies on
graph-based approaches, which are faster due to the graph simplification of
chemical structures. As such, cheminformatics is much used in many life
sciences fields, where the understanding of molecular properties and their
interactions with the cell and the organism. Therefore, the
...
\paragraph*{Results:}
We here report the improvements of the Chemistry Development Kit since the
1.2 release series. This paper outlines how the library evolved since the
previous report. We demonstrate the new APIs we introduced
