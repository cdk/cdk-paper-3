...
publications.
  ...
  an enumeration of the bond orders. This list of properties captures the
  ...
  IStereoElement was introduced with the implementations TetrahedralChirality,
...
\end{verbatim}
  ...
  representations~\cite{Faulon2004}.
...
\end{verbatim}
  ...
  and as methods on the DepictionGenerator class. This includes subscructure
  coloring, exemplified with this example reaction (see Fig.~\ref{fig:depiction}):
...
\end{verbatim}
  ...
  number of long standing issues. Particularly, collission detection has been
  greatly improved. Figure~\ref{fig:sdg} shows a difference in output between
...
The following code calculates all possible chemical formula for a given
accurate mass, within some allowed counts for each element:
...
\end{verbatim}
  ...
  and OpenSMILES specifications by one of the authors, and is independent of
  the CDK library. The SmilesParser API uses this library underneath, and the
  Beam API is hidden by this class.
...
the CDK previously included in aromaticty counting, which was limited in the 
...
required to be reviewed by one indepdent, senior CDK developer for  the development
...
OSGi bundles are available for the CDK too, which are used by Bioclipse and
KNIME.
...
\section*{Figures}
  \subsection*{Figure 1 - Dependencies between CDK modules.}\label{fig:deps}
...
\subsection*{Figure 2 - Atom type information specified for a sp3-hybridized
...
\subsection*{Figure 3 - The improved structure diagram generation has improved
...
\section*{Tables}
  ...
  \subsection*{Table 1 - A selection of key CDK modules with major changes.}\label{tab:modules}
