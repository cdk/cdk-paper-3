...
discuss the quality control approached we have adopted to ensure
stability. Additionally, new APIs have been introduced for atom type
perception, substructure searching, rendering of molecules, and handling of
molecular formulas.
...
\textit{BitSet} class shipping with Java. The benefits of the class include
...
\section*{Tables}
  \subsection*{Table 1 - The molecular fingerprints in CDK}
    \begin{tabular}{lccrc}
                             & Bit version  & Count version & Since CDK version & References \\
  \cline{2-5}
  Fingerprinter              & $\checkmark$ &               &                   & \\
  EStateFingerprinter        & $\checkmark$ &               &                   & \\
  ExtendedFingerprinter      & $\checkmark$ &               &                   & \\
  GraphOnlyFingerprinter     & $\checkmark$ &               &                   & \\
  HybridizationFingerprinter & $\checkmark$ &               &                   & \\
  KlekotaRothFingerprinter   & $\checkmark$ &               &                   & \\
  LingoFingerprinter         & $\checkmark$ &               &                   & \\
  MACCSFingerprinter         & $\checkmark$ &               &                   & \\
  PubchemFingerprinter       & $\checkmark$ &               &                   & \\
  ShortestPathFingerprinter  & $\checkmark$ &               &                   & \\
  SignatureFingerprinter     & $\checkmark$ & $\checkmark$  &                   & [pending] \\
  SubstructureFingerprinter  & $\checkmark$ &               &                   & \\
    \end{tabular}
